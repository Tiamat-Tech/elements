...
These atomic components can be used to assemble a custom &lt;Carousel&gt; component. Every rendered component accepts `className` and `style` props (as well as all other props accepted by a React DOM node) so extra CSS classes and styles may be applied to supplement the required styles provided in ther `styles.css` file, which contains the minimal CSS necessary for the carousel to function.
